...
async function runCrossAppActivity() {
  ...
  if (step === 0) {
    ...
    result = await callProgram({
      pid: TRUST_LAYER_PID,
      method: 'AgentTrustLayer/CreateEscrow',
      idl: TRUST_LAYER_IDL,
      value: process.env.CROSS_APP_ESCROW_VALUE || '0.05',
      args: [TRUST_LAYER_OWNER, SENTINEL_OPERATOR, `mainnet:${CONFIG.role}:trust-layer-v2:coordination-escrow:${sequence}:${current}`, deadlineBlock],
    });
  } else if (step === 1) {
    kind = 'trust-mission';
    result = await callProgram({
      ...
      args: [`Zenith routing mission ${sequence}`, `trust-suite://zenith/mission/${sequence}/${current}`, '50000000000', deadlineBlock, CONFIG.tags],
    });
  } else {
    ...
    result = await callProgram({
      ...
      idl: TRUST_MARKETPLACE_IDL,
      args: [TRUST_LAYER_OWNER, `trust-suite://zenith/hire/${sequence}/${current}`, '50000000000', deadlineBlock],
    });
  }
  ...
  await writeState(state);
  console.log(`Zenith cross-app ${kind} created:`, result);
}
