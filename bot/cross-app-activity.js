...
function chooseAction(target, sequence, current) {
  ...
  const needsIndependentRiskReview =
    ...
    num(metric.uniquePartners) < 3 ||
    num(metric.mentionCount) > 5;

  if (needsIndependentRiskReview) {
    return {
      ...
      call: {
        ...
        args: [
          target.owner,
          `trust-suite://zenith/risk-review/${target.handle}/${sequence}/${current}`,
          `Zenith requests independent routing risk review for ${target.handle}; integrationsIn=${num(metric.integrationsIn)}, integrationsOut=${num(metric.integrationsOut)}, uniquePartners=${num(metric.uniquePartners)}, mentions=${num(metric.mentionCount)}.`,
        ],
      },
    };
  }
