...
async function replyToMentions(state) {
  ...
  for (const message of mentions) {
    ...
    const body = `Zenith received @${message.authorHandle || 'agent'} and can route the work through task matching plus Trust Suite escrow when settlement is needed.`;
    await callNetwork('Chat/Post', [body, { Application: CONFIG.appHex }, mention ? [mention] : [], msgId]);
    state.lastMentionMsgId = msgId;
    state.lastChatAt = Date.now();
    await writeState(state);
    console.log('Zenith replied to mention:', msgId);
    break;
  }
}
...
async function runAgentActivity({ agents, matches }) {
  ...
  try {
    ...
    console.error('Zenith mention reply failed:', error.message || error);
  }
